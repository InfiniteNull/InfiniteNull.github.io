...
window.renderAiDataAnalyzer = function(container) {
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
        <button id="aiSubTabCsv" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-sky-600 text-white transition flex items-center gap-1.5">
          ...
          <span>Analisis Dataset CSV (Otomatis)</span>
        </button>
        <button id="aiSubTabText" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition flex items-center gap-1.5">
          ...
          <span>Uji Sentimen Teks Langsung</span>
        </button>
      </div>
        ...
        <div id="aiAnalysisResults" class="space-y-6">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm text-center">
              <div class="text-xs font-semibold text-slate-500 uppercase">Total Ulasan</div>
              <div id="aiStatTotal" class="text-2xl font-bold font-mono text-slate-900 dark:text-white mt-1">10</div>
            </div>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm text-center">
              <div class="text-xs font-semibold text-emerald-600 uppercase">Positif</div>
              <div id="aiStatPositive" class="text-2xl font-bold font-mono text-emerald-600 mt-1">7 (70%)</div>
            </div>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm text-center">
              <div class="text-xs font-semibold text-slate-500 uppercase">Netral</div>
              <div id="aiStatNeutral" class="text-2xl font-bold font-mono text-slate-500 mt-1">1 (10%)</div>
            </div>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm text-center">
              <div class="text-xs font-semibold text-red-500 uppercase">Negatif</div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col items-center justify-center">
              <h5 class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3 w-full text-left">Distribusi Sentimen</h5>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col">
              <h5 class="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3">Hasil Klasifikasi Baris per Baris</h5>
              <div class="overflow-x-auto flex-1 max-h-64 scrollbar-none">
                <table class="w-full text-left text-xs">
                  <thead class="bg-slate-50 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-semibold uppercase text-[10px] sticky top-0">
                    <tr>
                      <th class="p-2.5 rounded-l">No</th>
                      <th class="p-2.5">Teks Ulasan / Komentar</th>
                      <th class="p-2.5 text-right rounded-r">Prediksi Sentimen</th>
                    </tr>
                  </thead>
      ...
      <div id="aiTextPanel" class="hidden space-y-4">
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm space-y-4">
          <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300">Masukkan Ulasan / Kalimat Feedback untuk Dianalisis:</label>
          <textarea id="aiSingleTextInput" rows="3" class="w-full p-3 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-sky-500 outline-none resize-none" placeholder="Contoh: Layanan aplikasi sangat cepat, performa stabil dan antarmuka sangat mudah dipahami!"></textarea>
          ...
          <div class="flex justify-end">
            <button id="aiAnalyzeTextBtn" class="px-4 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-xs font-bold transition flex items-center gap-1.5">
              ...
              <span>Analisis Polarity Score</span>
            </button>
          </div>
        </div>
        ...
        <div id="aiSingleTextResult" class="hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-xs font-semibold text-slate-500">Hasil Analisis NLP:</span>
          ...
          <p class="text-xs text-slate-600 dark:text-slate-400" id="aiSingleExplanation">
            Algoritma VADER mendeteksi kata-kata bernada kepuasan tinggi seperti "cepat", "stabil", dan "mudah".
          </p>
        </div>
      </div>
  ...
  const positiveWords = ["bagus", "cepat", "profesional", "memuaskan", "ramah", "relevan", "bersih", "modern", "membantu", "terbaik", "mudah", "kompeten", "bersyukur", "mantap", "hebat", "puas", "andal", "great", "good", "excellent", "love", "awesome", "fast"];
  const negativeWords = ["lambat", "buruk", "kecewa", "rusak", "sulit", "padat", "kurang", "jelek", "gagal", "lelet", "parah", "kendala", "bad", "slow", "terrible", "poor", "hate", "worst", "error"];
  ...
  loadSampleBtn.addEventListener('click', () => {
    ...
    window.showToast?.("Berhasil memuat dataset ulasan contoh!");
  });
        ...
        Papa.parse(file, {
          ...
          complete: function(results) {
            if (results.data && results.data.length > 0) {
              ...
              window.showToast?.(`Berhasil menganalisis ${results.data.length} baris dari file ${file.name}`);
            }
          }
        });
      }
  ...
  analyzeTextBtn.addEventListener('click', () => {
    ...
    if (res.sentiment === "Positive") {
      ...
      singleExplanation.textContent = `Algoritma NLP mendeteksi sentimen kepuasan dan apresiasi positif dalam kalimat.`;
      ...
      singleExplanation.textContent = `Algoritma NLP mendeteksi keluhan atau masukan bernada evaluatif/negatif dalam kalimat.`;
    } else {
      ...
      singleExplanation.textContent = `Kalimat mengandung informasi umum tanpa kecenderungan emosi positif maupun negatif yang kuat.`;
    }
  });
